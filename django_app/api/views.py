...
from services.kafka_services import create_kafka_topic
from base.tasks import kafka_producer_task, calcul_distance_task
...
@api_view(['GET'])
def view_radar_by_id(request, id):
    radar = get_object_or_404(Radar, id=id)
    serializer = RadarSerializer(radar)
    return Response(serializer.data)
...
def run_kafka_producer_view(request):
    return kafka_producer_task(request)


@api_view(['GET'])
def calcul_distance_view(request):
    return calcul_distance_task(request)
